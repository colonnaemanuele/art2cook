...
def load_art2mus_pipeline(device):
    ...
    if device == 'cuda':
        ...
        pipe = AudioLDM2Pipeline.from_pretrained(pretrained_model_name_or_path=MODEL_REPO_ID, torch_dtype=torch.float16,
                                                 custom_pipeline=os.path.join(PROJ_DIR,"src/art2mus/art2mus_4_pipeline.py")).to(device)
    else:
...
def test_music_file_not_empty():
    """
    Test to check if music file is non-empty.
    """
    test_audio_path = os.path.join(PROJ_DIR, 'test_music/art2mus_example.wav')
    
    data, _ = sf.read(test_audio_path)
...
def test_similar_music():
    ...
    art2mus, generator = load_art2mus_pipeline(device)

    generate_audio(art2mus, generator, img_path=og_img_path, neg_prompt=NEG_PROMPT,
                   inf_steps=200, aud_len=10.0, waveforms=1, 
                   output_dir=TEST_AUD_OUT_DIR, file_name=f"/og_thistles_on_orange.wav")
    
    generate_audio(art2mus, generator, img_path=sim_img_path, neg_prompt=NEG_PROMPT,
                   inf_steps=200, aud_len=10.0, waveforms=1, 
                   output_dir=TEST_AUD_OUT_DIR, file_name=f"/og_california_thistles.wav")
    
    # Free up VRAM and space occupied by art2mus
    del art2mus
    ...
    aud_embeds = generate_embeds(imagebind, curr_dev=device, audio_paths=aud_paths, extract_emb=True, emb_type='audio')
    ...
    og_emb, variant_emb = aud_embeds[0], aud_embeds[1]
...
def test_partial_image():
    ...
    generate_audio(art2mus, generator, img_path=small_img_path, neg_prompt=NEG_PROMPT,
                   inf_steps=200, aud_len=10.0, waveforms=1, 
...
def test_brightness_variation():
    ...
    generate_audio(art2mus, generator, img_path=dark_img_path, neg_prompt=NEG_PROMPT,
                   inf_steps=200, aud_len=10.0, waveforms=1, 
